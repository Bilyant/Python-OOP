...
class Zoo:
    ...
    def get_entity_by_type(self, entity_name: str, obj):
        entities_of_given_type = []
        for entity in obj:
            if entity.__class__.__name__ == entity_name:
                entities_of_given_type.append(entity)
        return entities_of_given_type

    def animals_status(self):
        output = [f"You have {len(self.animals)} animals"]

        animals_of_type_lion = self.get_entity_by_type('Lion', self.animals)
        output.append(f'----- {len(animals_of_type_lion)} Lions:')
        for animal in animals_of_type_lion:
            output.append(str(animal))

        animals_of_type_tigers = self.get_entity_by_type('Tiger', self.animals)
        output.append(f'----- {len(animals_of_type_tigers)} Tigers:')
        for animal in animals_of_type_tigers:
            output.append(str(animal))

        animals_of_type_cheetahs = self.get_entity_by_type('Cheetah', self.animals)
